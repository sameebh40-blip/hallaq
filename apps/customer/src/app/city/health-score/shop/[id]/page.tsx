...
export default async function CityHealthScoreShopPage({ params }: { params: Promise<{ id: string }> }) {
  const t = await getT();
  const { id } = await params;
  const supabase = await createAppSupabaseServerClient();

  const [{ data: shop }, { data: scoreJson }] = await Promise.all([
    supabase.from("barbershops").select("id, name, logo_url, logo_path, rating_avg, rating_count, followers_count").eq("id", id).maybeSingle(),
    supabase.rpc("business_health_score", { p_entity_type: "shop", p_entity_id: id })
  ]);
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-md flex-col gap-4 px-4 pt-6 pb-24">
      <RealtimeRefresh tables={["bookings", "barbershops", "reviews", "follows"]} />
      ...
      <div className="overflow-hidden rounded-[28px] border bg-white p-4 shadow-[0_18px_48px_rgba(17,17,17,0.10)]">
        <div className="flex items-center gap-3">
          <div className="h-14 w-14 overflow-hidden rounded-2xl border bg-white">
            <SafeImage src={logo} fallbackKey="default_shop_logo" alt="" className="h-full w-full object-cover" />
          </div>
          <div className="flex flex-1 flex-col">
            <div className="text-[12px] font-semibold text-[#111111] line-clamp-1">{shop?.name ?? "Shop"}</div>
            <div className="mt-0.5 text-[11px] text-muted-foreground">
              ★ {Number(shop?.rating_avg ?? 0).toFixed(1)} • {Number(shop?.followers_count ?? 0).toLocaleString()} followers
            </div>
          </div>
        </div>
