...
export default async function BusinessNewBookingPage({
  searchParams
}: {
  ...
  if (!ctx.shop || !ctx.branch) {
    return (
      <LuxuryCard className="p-6">
        <div className="text-base font-semibold">New booking</div>
        <div className="pt-2 text-sm text-muted-foreground">Quick booking requires a shop + branch assignment.</div>
      </LuxuryCard>
    );
  }
  ...
  async function createReceptionBooking(formData: FormData) {
    ...
    if (!barberId || !serviceId || !startAt) redirect(`/business/bookings/new?error=${encodeURIComponent("Missing barber, service, or start time")}`);

    const supabase = await createAppSupabaseServerClient();
    const ctx = await getMyShopContext(supabase);
    if (!ctx.shop || !ctx.branch) redirect(`/business/bookings/new?error=${encodeURIComponent("No shop/branch assigned")}`);
    ...
    if (rpcError) redirect(`/business/bookings/new?error=${encodeURIComponent(rpcError.message)}`);
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <LuxuryCard className="p-5">
        ...
        {error ? <div className="mt-4 rounded-md border border-red-500/40 bg-red-500/10 p-3 text-sm text-red-200">{error}</div> : null}
      ...
      <LuxuryCard className="p-5">
        ...
        <div className="pt-2 text-sm text-muted-foreground">
          Bookings created here are scoped to branch_id and appear instantly across shop and barber schedules.
        </div>
      </LuxuryCard>
    </div>
  );
