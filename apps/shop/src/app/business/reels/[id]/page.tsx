...
export default async function BusinessReelDetailsPage({
  params,
  searchParams
}: {
  ...
  if (!reel || reel.shop_id !== shopId) notFound();
  ...
  async function save(formData: FormData) {
    ...
    if (thumbnailFile instanceof File && thumbnailFile.size > 0) {
      if (!(thumbnailFile.type ?? "").startsWith("image/")) redirect(`/business/reels/${id}`);
      const ctx = await getMyShopContext(supabase);
      if (!ctx.shop?.id) redirect(`/business/reels/${id}`);
      const ext = thumbnailFile.name.includes(".") ? thumbnailFile.name.split(".").pop() : undefined;
      const safeExt = ext ? `.${ext.toLowerCase()}` : "";
      const objectPath = `shops/${ctx.shop.id}/thumbnails/${id}/${randomUUID()}${safeExt}`;
      ...
      if (uploadError) redirect(`/business/reels/${id}?error=${encodeURIComponent(uploadError.message)}`);
      thumbnailPath = objectPath;
      thumbnailUrl = supabase.storage.from("reels").getPublicUrl(objectPath).data.publicUrl;
    }
    ...
    const { error: updateError } = await supabase
      ...
      .eq("id", id);
    if (updateError) redirect(`/business/reels/${id}?error=${encodeURIComponent(updateError.message)}`);
    if (actorId) {
  ...
  return (
    <div className="grid gap-4">
      <BusinessPageHeader
        title="Edit Reel"
        subtitle="Update caption, location, and thumbnail."
        actions={
