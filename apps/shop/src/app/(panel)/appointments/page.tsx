...
function userFacingDbError(message: string) {
  ...
  if (l.includes("permission denied") || l.includes("row-level security")) return "Permission denied. Make sure you are signed in to the correct shop account.";
  return m;
}
...
export default async function ShopAppointmentsPage({
  ...
  searchParams?: Promise<{ cursor?: string; status?: string; error?: string }>;
}) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {rawError ? (
        <LuxuryCard className="border border-red-500/40 bg-red-500/10 p-4 text-sm font-semibold text-red-200">{error}</LuxuryCard>
      ) : null}
      <LuxuryCard className="p-4">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div className="grid gap-1">
            <div className="text-base font-semibold">Appointments</div>
            <div className="text-sm text-muted-foreground">Manage upcoming bookings for your shop.</div>
          </div>
      ...
      {nextCursor ? (
        <div className="flex justify-center">
          <Button asChild variant="ghost" size="sm">
            <a
              href={`/appointments?cursor=${encodeURIComponent(nextCursor)}&status=${encodeURIComponent(
                statusFilter || "all"
              )}`}
            >
              Load more
