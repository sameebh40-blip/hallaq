...
export default async function ShopServicesPage({
  ...
  async function updateAssignments(formData: FormData) {
    ...
    const selected = formData.getAll("assigned_barbers").map((v) => String(v)).filter(Boolean);

    const supabase = await createAppSupabaseServerClient();
    const shop = await getMyShop(supabase);
    if (!shop) redirect("/services");
  ...
  async function togglePopular(formData: FormData) {
    ...
    if (upError) redirect(`/services?error=${encodeURIComponent(upError.message)}`);
    if (actorId) {
      await supabase.from("audit_events").insert({
        actor_profile_id: actorId,
  ...
  async function updateService(formData: FormData) {
    ...
    if (!id || !nameEn) redirect("/services");
  ...
  return (
    <div className="flex flex-col gap-6">
      {params?.error ? (
        <LuxuryCard className="p-4 border border-red-500/40 bg-red-500/10">
        ...
        <div className="overflow-x-auto">
                  ...
                  <tr key={r.id} className="hover:bg-white/5">
                    ...
                    <td className="px-4 py-3">
                      ...
                      {r.description_en || r.description ? (
                        <div className="text-xs text-muted-foreground">{r.description_en ?? r.description}</div>
                      ) : null}
                    </td>
                    ...
                    <td className="px-4 py-3 text-muted-foreground">{r.duration_minutes ?? r.duration_min} min</td>
                    <td className="px-4 py-3 text-muted-foreground">{Number(r.price_bhd ?? r.price ?? 0).toFixed(3)} BHD</td>
                    <td className="px-4 py-3 text-muted-foreground">{r.is_popular ? "Yes" : "No"}</td>
                    <td className="px-4 py-3 text-muted-foreground">{r.is_active ?? r.active ? "Yes" : "No"}</td>
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          <form action={toggleActive}>
                            ...
                            <input
                              type="hidden"
                              name="current"
                              value={String(Boolean(r.is_active ?? r.active))}
                            />
                            <Button type="submit" size="sm" variant="secondary">
                        ...
                        >
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <Input
                              ...
                              type="number"
                              step="0.001"
                              defaultValue={Number(r.price_bhd ?? r.price ?? 0)}
                            />
                            <Input
                              name="duration_minutes"
                              type="number"
                              defaultValue={Number(r.duration_minutes ?? r.duration_min ?? 30)}
                            />
                          </div>
                          <div className="grid grid-cols-2 gap-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <Input
              ...
              defaultValue={Number(r.deposit_value ?? 0)}
            />
          </div>
                        ...
                        {r.barber_id ? null : (
                          ...
                          >
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              {(barbers ?? []).map((b) => {
                                const checked =
                                  (assignments ?? []).some((a) => a.service_id === r.id && a.barber_id === b.id);
                                return (
                                  <label key={b.id} className="flex items-center gap-2 text-sm">
                                    <input
                                      type="checkbox"
                                      name="assigned_barbers"
                                      value={b.id}
